...
const baseList = [
    ...Array.from({ length: VISIBLE_LENGTH }, () => getRandomInt(1, 3)),    // creating a list with 2 parts, one with random numbers between 1 and 3
    ...Array(BASE_LENGTH - VISIBLE_LENGTH).fill(0)                          // the other part filled with zeros
];                                                                
...
function updateValue(index, delta = 0, newValue = null) {   // update value in the baseList
...
function attachHoldHandler(button, index, direction) {
    ...
    const step = () => {
        ...
        if (prev !== next) {    // Only render if the value actually changed
            renderListAndSet();
        } else {
            stop();
        }
...
function renderList(length) {                                   // called everytime a portion of the list needs to be graphically rendered
    ...
    numListEl.innerHTML = '';                                   // empties the 'num-list' div
    ...
    displayList.forEach((value, index) => {                     // looping through each value and it's respective index
        const wrapper = document.createElement('div');          // creating a wrapper that'll include arrows on top and bottom of <input>
